...
class MultiAttention(nn.Module):
    def __init__(self, input_size=1024, output_size=1024, freq=10000, pos_enc=None,
                 num_segments=None, heads=1, fusion=None, no_global=False, no_residual=False):
    ...
    def forward(self, x):
        ...
        """
        if self.no_global == True:
            weighted_value = torch.zeros_like(x)
            attn_weights = torch.zeros((x.size(0), x.size(0)))
        else:
...
class PGL_SUM(nn.Module):
    def __init__(self, input_size=1024, output_size=1024, freq=10000, pos_enc=None,
                 num_segments=None, heads=1, fusion=None, no_global=False, no_residual=False, no_attention_dropout=False, no_attention_norm=False, no_second_norm=False):
        ...
        self.attention = MultiAttention(input_size=input_size, output_size=output_size, freq=freq,
                                        pos_enc=pos_enc, num_segments=num_segments, heads=heads, fusion=fusion, no_global=no_global, no_residual=no_residual)
        ...
        self.no_residual = no_residual
        self.no_attention_dropout = no_attention_dropout
        self.no_attention_norm = no_attention_norm
        self.no_second_norm = no_second_norm

    def forward(self, frame_features):
        """ Produce frames importance scores from the frame features, using the PGL-SUM model.

        :param torch.Tensor frame_features: Tensor of shape [T, input_size] containing the frame features produced by the 
            mixed5c layer of I3D.
        :return: A tuple of:
        ...
        if self.no_residual == True:
            y = weighted_value
        else:
            y = weighted_value + residual
        if self.no_attention_dropout != True:
            y = self.drop(y)
        if self.no_attention_norm != True:
            y = self.norm_y(y)
